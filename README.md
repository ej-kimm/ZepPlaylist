...
<img src="https://github.com/user-attachments/assets/d860545a-18d7-4768-87ba-08fd8782f56a" alt="logo" width="500">
...
| :---------------------------------------------------------------------------------------------------------------: | :--------------------------------------------------------------------------------------------------------------: | :---------------------------------------------------------------------------------------------------------------: | :--------------------------------------------------------------------------------------------------------------: | :---------------------------------------------------------------------------------------------------------------: | :---------------------------------------------------------------------------------------------------------------: |
|                        **팀장**<br/>주호빈<br/>[@Hobin-joo](https://github.com/Hobin-joo)                         |                          **부팀장**<br/>김은지<br/>[@ej-kimm](https://github.com/ej-kimm)                          |                           **팀원**<br/>김지은<br/>[@zzieni](https://github.com/zzieni)                            |                           **팀원**<br/>박준석<br/>[@bj9322](https://github.com/bj9322)                           |                          **팀원**<br/>권현준<br/>[@KHY3260](https://github.com/KHY3260)                           |                     **팀원**<br/>김윤아<br/>[@KimYoona0527](https://github.com/KimYoona0527)                      |
...
<img src="https://github.com/user-attachments/assets/4f9d5b13-36bc-4fe9-8ba5-c7bae6dc4704" alt="ERD" width="700">
...
- 개별 곡 재생 및 두 곡 이상(플레이리스트) 연속 재생을 지원합니다
- 저작권 이슈로 인해 **하이라이트 30초만 재생** 가능합니다
- 음악 재생, 일시정지, 다음 곡, 이전 곡 넘기기가 가능합니다
- **다른 페이지로 이동해도 음악이 끊기지 않고 계속 재생됩니다**

#### 🖼 음악 정보 제공

- 앨범 커버 클릭 시 곡 정보(제목, 아티스트, 발매일, 장르)를 확인할 수 있습니다

#### ❤️ 곡별 인터랙션 기능

- 해당 곡에 대해 좋아요를 누를 수 있습니다
- 플레이리스트에 추가할 수 있습니다

#### 📝 가사 및 번역 지원

- 기본적으로 **3줄 가사**를 제공하며, 클릭 시 전체 가사를 확인할 수 있습니다
- 가사 번역(한국어) 기능을 제공합니다

#### 🔊 볼륨 조절

- 사용자가 직접 노래 볼륨을 조절할 수 있습니다
...
- ✨ API 호출 횟수 감소
  ...
  - 전체재생 실행 마다 반복적인 API 호출에서 하루 24회로 Spotify API 호출 횟수 감소
  - HTTP 429 에러 발생 빈도 현저히 감소

- ✨ 성능 향상
  - 전체 재생 기능의 로딩 시간 약 70% 단축
  - 사용자 경험 개선 및 서버 부하 감소
- ✨ 데이터 일관성 향상
  - 모든 사용자에게 동일한 최신 차트 데이터 제공
  - 캐시와 실제 데이터 간의 불일치 문제 해결

#### 5. 배운 점

1. GitHub Actions의 활용

- 주기적 작업 자동화의 중요성 인식
- CI/CD 파이프라인 구축 경험 획득

2. 캐싱 전략의 중요성:

- 적절한 캐싱을 통한 성능 최적화 방법 학습
- API 요청 최소화의 중요성 이해

3. 시스템 설계 개선:

- 사용자 경험과 서버 리소스 관리의 균형 유지 방법 학습

### 5️⃣ 플레이리스트 하트(좋아요) 버튼 UI 동기화 문제 해결

#### 1. 문제 상황

- 웹 플레이리스트 카드에서 하트(좋아요) 버튼을 클릭했을 때, 해당 플레이리스트의 좋아요 상태가 즉시 UI에 반영되지 않고 항상 채워진 하트 아이콘이 표시되었으며, 새로고침 후에야 정상적으로 업데이트되었다

#### 2. 원인 분석

- `useToggleLike` 훅에서 좋아요 상태를 관리하는 playlist_like 캐시 데이터만 갱신하고, 플레이리스트 목록(playlists) 캐시는 갱신하지 않아 UI와 데이터가 동기화되지 않았다

#### 3. 해결 과정

- 좋아요 토글 실행 후, `queryClient.invalidateQueries`를 사용하여 playlist_like뿐만 아니라 playlists 캐시도 갱신하도록 수정하여 UI가 정상적으로 업데이트되도록 개선하였다

#### 4. 배운 점

- react-query 사용 시, 단순히 데이터를 불러오는 것뿐 아니라 언제, 어떤 데이터를 갱신해야 하는지 고민해야 하며, invalidateQueries를 통한 적절한 캐시 관리와 최적화가 UI 동기화와 불필요한 재렌더링 방지에 핵심적임을 깨달았다
